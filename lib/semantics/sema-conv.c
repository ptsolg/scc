...
static int c_get_type_rank(const tree_type* t)
{
        ...
        c_sema* self, tree_type* lhs, tree_type* rhs)
{
        ...
        size_t lsize = tree_get_builtin_type_size(self->target, lk);
        size_t rsize = tree_get_builtin_type_size(self->target, rk);
        if (rsize > lsize)
                return rhs;
